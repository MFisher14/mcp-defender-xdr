...
from mcp_defender_xdr.auth import (
    AzureCredentials,
...
from mcp_defender_xdr.errors import AuthError
...
def test_credentials_repr_redacts_passphrase() -> None:
    creds = AzureCredentials(
        tenant_id="t", client_id="c", cert_path="/p/cert.pfx", cert_passphrase="super-secret"
    )
    rendered = repr(creds)
    assert "super-secret" not in rendered
    assert "REDACTED" in rendered
    assert "/p/cert.pfx" in rendered  # path is fine to log


def test_credentials_repr_when_no_passphrase() -> None:
    creds = AzureCredentials(tenant_id="t", client_id="c", cert_path="/p/cert.pfx")
    rendered = repr(creds)
...
def test_credentials_authority_url() -> None:
    creds = AzureCredentials(tenant_id="abc", client_id="c", cert_path="/p/c.pfx")
...
def test_build_client_credential_with_passphrase(tmp_path: Path) -> None:
    ...
    creds = AzureCredentials(tenant_id="t", client_id="c", cert_path=str(pfx), cert_passphrase="pw")
...
def test_build_client_credential_without_passphrase(tmp_path: Path) -> None:
    pfx = tmp_path / "c.pfx"
    pfx.write_bytes(b"\x00")
    creds = AzureCredentials(tenant_id="t", client_id="c", cert_path=str(pfx))
    cc = _build_client_credential(creds)
...
def test_env_provider_passphrase_optional(tmp_path: Path) -> None:
    ...
    creds = EnvCredentialProvider(env).get_credentials()
    assert creds.cert_passphrase is None


@pytest.mark.parametrize(
    "missing",
    ["AZURE_TENANT_ID", "AZURE_CLIENT_ID", "AZURE_CERT_PATH"],
)
...
def test_json_provider_happy_path(tmp_path: Path) -> None:
    ...
    assert fabrikam.tenant_id.startswith("3333")
...
def test_json_provider_rejects_world_readable_file(tmp_path: Path) -> None:
    ...
    cfg = _write_tenants_file(
        ...
        {
            "default": "contoso",
            "tenants": {"contoso": {"tenant_id": "t", "client_id": "c", "cert_path": str(pfx)}},
        },
        mode=0o644,
    )
    with pytest.raises(AuthError) as exc_info:
...
def test_json_provider_invalid_tenant_key(tmp_path: Path) -> None:
    ...
    cfg = _write_tenants_file(
        ...
        {
            "default": "bad key",
            "tenants": {"bad key": {"tenant_id": "t", "client_id": "c", "cert_path": str(pfx)}},
        },
    )
    with pytest.raises(AuthError) as exc_info:
        JsonFileCredentialProvider(cfg, env={})
...
def test_token_manager_per_tenant_cache_isolation() -> None:
    ...
    from mcp_defender_xdr.auth import CredentialProvider

    class TwoTenantProvider(CredentialProvider):
        def __init__(self) -> None:
            self._creds = {
                "a": AzureCredentials(tenant_id="aaa", client_id="ca", cert_path="/dev/null"),
                "b": AzureCredentials(tenant_id="bbb", client_id="cb", cert_path="/dev/null"),
            }
    ...
    assert manager.get_token("a") == "token-ca"  # still cached
